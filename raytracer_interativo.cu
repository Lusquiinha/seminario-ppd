...
__host__ __device__ static inline Vec3f vec_add(Vec3f a, Vec3f b) {
    Vec3f r = { a.x + b.x, a.y + b.y, a.z + b.z };
    return r;
}

__host__ __device__ static inline Vec3f vec_sub(Vec3f a, Vec3f b) {
    Vec3f r = { a.x - b.x, a.y - b.y, a.z - b.z };
    return r;
}

__host__ __device__ static inline Vec3f vec_mul_scalar(Vec3f a, float s) {
    Vec3f r = { a.x * s, a.y * s, a.z * s };
    return r;
}

__host__ __device__ static inline Vec3f vec_mul(Vec3f a, Vec3f b) {
    Vec3f r = { a.x * b.x, a.y * b.y, a.z * b.z };
    return r;
}

__host__ __device__ static inline float vec_dot(Vec3f a, Vec3f b) {
    return a.x * b.x + a.y * b.y + a.z * b.z;
}

__host__ __device__ static inline float vec_length(Vec3f a) {
    return sqrtf(vec_dot(a, a));
}

__host__ __device__ static inline Vec3f vec_normalize(Vec3f a) {
...
__host__ __device__ static inline Vec3f vec_neg(Vec3f a) {
    Vec3f r = { -a.x, -a.y, -a.z };
    return r;
}

__host__ __device__ static inline Vec3f vec_cross(Vec3f a, Vec3f b) {
    Vec3f r = {
        a.y * b.z - a.z * b.y,
        a.z * b.x - a.x * b.z,
        a.x * b.y - a.y * b.x
...
__host__ __device__ static inline float mixf(float a, float b, float mix) {
    return b * mix + a * (1.0f - mix);
}

__host__ __device__ static inline float clampf(float x, float a, float b) {
    if (x < a) return a;
    if (x > b) return b;
    return x;
}

/* ======== Memória constante para esferas ======== */
__constant__ Sphere d_const_spheres[MAX_SPHERES];
...
__device__ bool sphere_intersect(const Sphere *s, Vec3f rayorig, Vec3f raydir, float *t0, float *t1) {
...
__device__ Vec3f trace(Vec3f rayorig,
            Vec3f raydir,
...
/* ======== Kernel CUDA para renderização ======== */

__global__ void render_kernel(Vec3f *image,
                              unsigned width,
                              unsigned height,
                              int numSpheres,
                              Camera cam,
                              ...
                              float angleAspect)
{
    ...
    float xx = (2.0f * ((x + 0.5f) * invWidth)  - 1.0f) * angleAspect;
    ...
    // Origem do raio é a posição da câmera (usa memória constante para esferas)
    image[y * width + x] = trace(cam.pos, raydir, d_const_spheres, numSpheres, 0);
}
            ...
            const Camera *cam)
{
    ...
    float angleAspect = angle * aspectratio;
    
    // Configuração dos blocos e threads (otimizado para warps)
    dim3 blockSize(16, 16);
    dim3 gridSize((width + blockSize.x - 1) / blockSize.x,
                  (height + blockSize.y - 1) / blockSize.y);
    
    // Executa kernel
    render_kernel<<<gridSize, blockSize>>>(d_image, width, height, numSpheres, *cam, 
                                           invWidth, invHeight, angle, angleAspect);
    
    // Sincroniza antes de copiar
...
int main(int argc, char *argv[]) {
    ...
    if (numSpheres > MAX_SPHERES) {
        fprintf(stderr, "Erro: Número de esferas (%d) excede o máximo (%d)\n", numSpheres, MAX_SPHERES);
        free(spheres);
    ...
    // Aloca memória na GPU (uma vez antes do loop)
    Vec3f *d_image;
    cudaMalloc(&d_image, WIDTH * HEIGHT * sizeof(Vec3f));
    
    // Copia esferas para memória constante (mais rápida, apenas uma vez)
    cudaMemcpyToSymbol(d_const_spheres, spheres, numSpheres * sizeof(Sphere));
    ...
    while (running) {
        ...
        // Mostra FPS no terminal (atualiza a cada segundo)
        frameCount++;
