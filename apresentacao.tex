...
\begin{frame}{Teste 2: Modo Interativo - Comparação Tripla}
    ...
    \begin{center}
        \begin{tabular}{lccc}
            ...
            FPS Médio & 6.89 & 45.23 & 177.84 \\
            FPS Máximo & 19.83 & 118.69 & 228.09 \\
            Desvio Padrão & 4.87 & 32.14 & 24.87 \\
            \midrule
            \textbf{Speedup vs Seq} & \textbf{1.00x} & \textbf{6.56x} & \textbf{25.81x} \\
            \bottomrule
        \end{tabular}
    \end{center}
\end{frame}
...
\begin{frame}{Análise de Performance - FPS Interativo}
    \begin{columns}
        ...
        \begin{block}{OpenMP}
            \begin{itemize}
                \item Média: \textbf{45.23 FPS}
                \item Usável
                \item Instabilidade
                \item \textbf{6.56x} speedup
            \end{itemize}
        \end{block}
        ...
        \begin{block}{CUDA}
            \begin{itemize}
                \item Média: \textbf{177.84 FPS}
                \item Muito fluido
                \item Consistente
                \item \textbf{25.81x} speedup
            \end{itemize}
        \end{block}
    \end{columns}
    ...
    \begin{alertblock}{Destaque}
        CUDA é \textbf{3.93x} mais rápido que OpenMP e \textbf{25.81x} mais rápido que Sequencial
    \end{alertblock}
\end{frame}

\begin{frame}{Visualização: FPS ao Longo do Tempo}
    \begin{center}
        \textbf{Gráfico gerado com Streamlit}
        ...
        \fbox{
            \begin{minipage}{0.9\textwidth}
                \centering
                \vspace{2cm}
                \textit{[Inserir gráfico de linha do Streamlit]}
                
                \textit{comparar\_fps.py - FPS vs Tempo}
                \vspace{2cm}
            \end{minipage}
        }
        ...
        \textbf{Observações:}
        \begin{itemize}
            \item Sequencial: Performance extremamente baixa e inconsistente
            \item OpenMP: Grande variação devido a scheduling dinâmico
...
\begin{frame}{Visualização: Distribuição de FPS}
    \begin{center}
        \textbf{Histogramas gerados com Streamlit}
        ...
        \fbox{
            \begin{minipage}{0.9\textwidth}
                \centering
                \vspace{2.5cm}
                \textit{[Inserir histogramas lado a lado do Streamlit]}
                
                \textit{comparar\_fps.py - Distribuição de FPS}
                \vspace{2.5cm}
            \end{minipage}
        }
...
\begin{frame}[fragile]{Teste em Andamento: Schedule Static vs Dynamic}
    ...
    \begin{alertblock}{Status}
        Teste planejado - resultados serão adicionados
    \end{alertblock}
\end{frame}

\begin{frame}{Placeholder: Resultados Schedule Static}
    \begin{center}
        \fbox{
            \begin{minipage}{0.85\textwidth}
                \centering
                \vspace{2cm}
                \textit{[Espaço reservado para gráfico Streamlit]}
                
                \textbf{Comparação: Dynamic vs Static Scheduling}
                
                \textit{Dados serão adicionados após execução do teste}
                \vspace{2cm}
            \end{minipage}
        }
...
\begin{frame}{Conclusões}
    ...
    \begin{alertblock}{Resultados Quantitativos}
        \begin{itemize}
            ...
            \item \textbf{Interativo Sequencial}: 6.89 FPS - Inviável
            \item \textbf{Interativo OpenMP}: 45.23 FPS - Speedup 6.56x
            \item \textbf{Interativo CUDA}: 177.84 FPS - Speedup 25.81x vs Seq / 3.93x vs OMP
...
\begin{frame}{Trabalhos Futuros}
    \begin{itemize}
        \item \textbf{Completar testes de scheduling}:
        \begin{itemize}
            \item Static vs Dynamic no modo interativo
            \item Guided e outras políticas
            \item Chunk size otimizado
        \end{itemize}
